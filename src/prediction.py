...
class Predictor:
    ...
    def __init__(self, model_name: str, model_checkpoint: Optional[str], device: str, config: Config) -> None:
        ...
        self._device = device
        # load access token
        with open('huggingface_access_token.txt', 'r') as fin:
            hf_token = fin.read().strip('\n')
        pred_logger.info('Load tokenizer.')
        self._tokenizer = AutoTokenizer.from_pretrained(model_name, use_auth_token=hf_token)
        pred_logger.info(f'Load model {model_name} from checkpoint {model_checkpoint}.')
        self._model = AutoModelForSequenceClassification.from_pretrained(
            model_checkpoint if model_checkpoint else model_name, use_auth_token=hf_token
        )
    ...
    def nli_classify_bin(self, input_text: str, hypothesis: str) -> float:
        ...
        self._lengths_seen.append(len(encoded_input[0]))
    ...
    def classify_no_nli(self, input_text) -> List[float]:
        ...
        """
        encoded_input = self._tokenizer.encode(input_text, return_tensors='pt', truncation='only_first', max_length=128)
        logits = self._model(encoded_input.to(self._device))[0]
...
class PredictionPipeline:
    ...
    def __init__(self, predictors: Dict[str, Predictor], config: Config):
        ...
        for id_ in config['prediction_pipeline']['catchers']:
            if len(predictors) == 1:
                predictor = list(self._predictors.values())[0]
                self._catcher_insts.append(self._catcher_clss[id_](predictor, config))
            else:
                if id_ == 'HSCatcher':
                    self._catcher_insts.append(self._catcher_clss[id_](self._predictors['main'], config))
                else:
                    self._catcher_insts.append(self._catcher_clss[id_](self._predictors['aux'], config))
        for id_ in config['prediction_pipeline'].get('filters', []):
            if len(predictors) == 1:
                predictor = list(self._predictors.values())[0]
                self._filter_insts.append(self._filter_clss[id_](predictor, config))
            else:
                self._filter_insts.append(self._filter_clss[id_](self._predictors['aux'], config))
